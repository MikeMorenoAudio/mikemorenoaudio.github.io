...
The first tests of voice activity detection (VAD) were successful in a controlled environment with a combination of volume threshold using the `[env~]` object and zero-crossing rate threshold using the `[zeroCrossing~]` object from William Brent’s timbreID library.
